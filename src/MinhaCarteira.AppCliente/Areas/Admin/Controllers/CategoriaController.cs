...
public class CategoriaController(ICategoriaRefit servico, IHttpContextAccessor httpContextAccessor)
    : Base.BaseAdminController<CategoriaViewModel, Guid, ICategoriaRefit>(servico, httpContextAccessor)
{
    ...
    public override async Task<JsonResult> ObterTodos(string prefix)
    {
        ...
        {
            NomePropriedade = "caminho",
            Operador = TipoOperadorBusca.Contem,
            Valor = prefix
        });

        var view = await this.ChamarServicoPaginado<CategoriaViewModel, Guid, ICategoriaRefit>(criterio, false);
        ...
        var retornoApi = result.Model as ListaBaseViewModel<CategoriaViewModel>;
        var items = retornoApi?.Itens
            .Select(s => new
            {
                icone = string.IsNullOrEmpty(s.Icone.Conteudo)
                    ? ""
                    : $"data:{s.Icone.MimeType};base64,{s.Icone.Conteudo}",
                label = s.Caminho,
                val = s.Id
            })
